...
public enum Distance {
    WALK("Walk", "Walk", Scale.LINEAR) {
        @Override
        public synchronized DoubleMatrix getD(DoubleMatrix A, double alpha) {
    ...
    LOGARITHMIC_FOREST("Logarithmic Forest", "logFor", Scale.LOG1) {
        @Override
        public synchronized DoubleMatrix getD(DoubleMatrix A, double t) {
            DistancesBuilder db = new DistancesBuilder();
    ...
    COMBINATIONS("Convex combination of the shortest path and resistance metrics", "SP-CT", Scale.LINEAR) {
        @Override
        public synchronized DoubleMatrix getD(DoubleMatrix A, double lambda) {
    ...
    HELMHOLTZ_FREE_ENERGY("Helmholtz Free Energy Distances", "FE", Scale.LOG2) {
        @Override
        public synchronized DoubleMatrix getD(DoubleMatrix A, double beta) {
            DistancesBuilder db = new DistancesBuilder();
